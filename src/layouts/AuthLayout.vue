  ...
  <div class="auth-layout grid grid-cols-12 content-center">
    <div class="flex col-span-12 p-4 justify-center">
      <router-link class="py-5 justify-center flex" to="/">
        <vuestic-logo style="width: 150px; height: 150px;" />
      </router-link>
    </div>
    ...
    <div class="flex justify-center col-span-12 p-4">
      <va-card class="auth-layout__card">
        <va-card-content>
          <va-tabs color="#ff842b" v-model="tabIndex" center>
            <template #tabs >
              <va-tab color="#ff842b" name="login">{{ t('auth.login') }}</va-tab>
              <va-tab color="#ff842b" name="signup">{{ t('auth.createNewAccount') }}</va-tab>
            </template>
          </va-tabs>
